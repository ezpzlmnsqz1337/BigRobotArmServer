...
class StatusCommand(AbstractCommand):
  ...
  def parse_response(self, response):
    data = response.split("\n")
    stream = BitStream(f"0x{data[1]}", )

    p_base = stream.read("int:32")
    p_shoulder = stream.read("int:32")
    p_elbow = stream.read("int:32")
    p_wrist_rotate = stream.read("int:32")
    p_wrist = stream.read("int:32")
    result = f"BigRobotArm::POSITION: B{p_base} S{p_shoulder} E{p_elbow} WR{p_wrist_rotate} W{p_wrist}\n"

    a_base = stream.read("int:32")
    a_shoulder = stream.read("int:32")
    a_elbow = stream.read("int:32")
    a_wrist_rotate = stream.read("int:32")
    a_wrist = stream.read("int:32")
    result += f"BigRobotArm::ACCELERATION: B{a_base} S{a_shoulder} E{a_elbow} WR{a_wrist_rotate} W{a_wrist}\n"

    s_base = stream.read("int:32")
    s_shoulder = stream.read("int:32")
    s_elbow = stream.read("int:32")
    s_wrist_rotate = stream.read("int:32")
    s_wrist = stream.read("int:32")
